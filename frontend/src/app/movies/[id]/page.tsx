...
export default function MovieDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isInWatchlist = watchlistData?.watchlist?.some((item: any) => item.contentId === parseInt(resolvedParams.id) && item.contentType === 'movie') ?? false;
    ...
    mutateWatchlist(
      {
        watchlist: newIsInWatchlist
          ? [...(watchlistData?.watchlist || []), { contentId: parseInt(resolvedParams.id), contentType: 'movie' }]
          : (watchlistData?.watchlist || []).filter((item: any) => !(item.contentId === parseInt(resolvedParams.id) && item.contentType === 'movie'))
      },
      false
    );
    ...
    try {
      ...
      const response = await fetch(
        newIsInWatchlist ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/watchlist` : `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/watchlist/${resolvedParams.id}`,
        {
          ...
          body: newIsInWatchlist ? JSON.stringify({ contentId: parseInt(resolvedParams.id), contentType: 'movie' }) : undefined,
        }
      );
